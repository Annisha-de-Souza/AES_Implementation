...
if __name__ == "__main__":
    ...
    print("Enter the plaintext in hexadecimal format (e.g., 00112233445566778899aabbccddeeff):")
    plaintext_input = input().strip()

    print("Enter the key in hexadecimal format (e.g., 000102030405060708090a0b0c0d0e0f):")
    key_input = input().strip()

    # Convert input strings to byte arrays
    try:
        plaintext = bytearray.fromhex(plaintext_input)
        key = bytearray.fromhex(key_input)
    except ValueError:
        print("Invalid hexadecimal input. Please provide valid hexadecimal values.")
        exit()

    # Validate key size (should be 16 bytes for AES-128, 24 bytes for AES-192, or 32 bytes for AES-256)
    if len(key) not in [16, 24, 32]:
        print("Invalid key size. The key must be 16 bytes (AES-128), 24 bytes (AES-192), or 32 bytes (AES-256).")
        exit()
